...
$stmt = $conn->prepare('SELECT student_num, password FROM student WHERE student_num = ? ');
...
if ($stmt->num_rows > 0) {
    ...
    $stmt = $conn->prepare("INSERT INTO student (student_num, firstname, surname, email, dob, address, postcode, password) VALUES(?, ?, ?, ?, ?, ?, ?, ?);");
